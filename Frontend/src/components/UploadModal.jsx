...
export default function UploadModal({ onClose, onErgebnis }) {
  ...
  async function handleBestaetigen() {
    ...
    try {
      ...
      const student = await api.studentSuchen(matrikelnummer.trim());
      setStudentId(student.id);
      // Gefunden → direkt auswerten
      await speichernUndAbschliessen(student.id);
    } catch (err) {
  ...
  async function handleStudentAnlegenUndSpeichern() {
    ...
    setFehler(null);
    try {
      const student = await api.studentAnlegen(studentDialog.matNr, studentName);
      setStudentId(student.id);
